...
app.config['MYSQL_DATABASE_USER'] = sql_config["user"]
app.config['MYSQL_DATABASE_PASSWORD'] = sql_config["password"]
app.config['MYSQL_DATABASE_DB'] = sql_config["database"]
app.config['MYSQL_DATABASE_HOST'] = sql_config["host"]
...
@app.route('/mygarden/watering_graph')
def api_watergraph():
  return "Watering Graph Here"

#calculate average temperature by weeks
@app.route('/mygarden/temperature_graph', methods=['GET'])
def api_tempgraph():
...
@app.route('/mygarden/temperature_graph/individual', methods=['GET'])
...
@app.route('/sensor_table', methods=['GET'])
def api_sensors():
