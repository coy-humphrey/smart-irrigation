...
void setup() {
  ...
  size(1280, 300); // Processing export to JS barfs on the line above so changed to this
  ...
  for (int i = 0; i < MAX_FLOWERS; i++) {
    ...
    // grow or shrink flowers
      if (flowers[i][GROW] < 1) { // shrinking state
        flowers[i][SIZE] -=0.50; // shrink 8x faster than growth
        if (flowers[i][SIZE] <= 0) {
        ...
        if (flowers[i][SIZE] >= MAX_FLOWER_SIZE) {
          ...
          flowers[i][GROW] -= 0.50; // change direction of growth next frame
        }
      }
      ...
      rotate((i/float(MAX_FLOWERS - 1)) * PI / 2.0 + step * PI / 700.0);
